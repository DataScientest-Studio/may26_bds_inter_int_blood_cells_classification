...
------------------------------------------------------------------------
Corrections applied in this version (see CHANGELOG at bottom):
  1. class_weights.json now round-trips to INT keys via load_class_weights();
     JSON stores keys as strings, and Keras model.fit(class_weight=...) needs
     integer keys, so consumers MUST load it through the helper below.
  2. build_tf_dataset() takes an explicit `normalization` mode. For a Keras
     ImageNet backbone (e.g. MobileNetV2) use "tf" ([-1, 1]); the PyTorch-style
     mean/std path is available as "imagenet" but is NOT what keras.applications
     backbones expect.
  3. IMAGE_SIZE is documented and used consistently as (height, width)
     everywhere, with cv2.resize receiving (width, height) explicitly.
  4. Arrays handed to tf.data.Dataset.from_tensor_slices are materialised as
     plain numpy (object/int32), which is robust under pandas >= 3.0 where
     Series.values can return a StringArray rather than an ndarray.
------------------------------------------------------------------------
"""
...
# IMAGE_SIZE is always interpreted as (height, width).
...
# PyTorch/torchvision ImageNet statistics (assume inputs already scaled to 0-1).
# NOTE: keras.applications backbones (MobileNetV2, etc.) do NOT expect these.
# They expect their own preprocess_input, which maps pixels to [-1, 1]
# (normalization="tf" below). Keep these only if you deliberately want the
# torch-style normalisation.
IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD = [0.229, 0.224, 0.225]

# Supported normalisation modes for the tf.data pipeline.
#   "zero_one" -> pixels scaled to [0, 1]              (from-scratch CNN / dense)
#   "tf"       -> pixels scaled to [-1, 1]             (keras.applications backbones)
#   "imagenet" -> (x/255 - mean) / std, torch stats    (only for torch-style models)
NORMALIZATION_MODES = ("zero_one", "tf", "imagenet")
DEFAULT_NORMALIZATION = "zero_one"
            ...
            rows.append(
                {
                    ...
                    # store the relative path with POSIX separators so the
                    # manifest is portable and re-joinable on any platform.
...
def create_label_map(df: pd.DataFrame) -> Dict[str, int]:
    """Create class-name to integer-label mapping (alphabetical, stable)."""
...
def stratified_split(
    ...
    First split carves off `train_ratio`; the remainder is divided between
    validation and test so that both keep their requested share of the whole.
    """
    ...
    # fraction of the held-out remainder that should become the test set
...
def calculate_class_weights(train_df: pd.DataFrame) -> Dict[int, float]:
    ...
    Returns a dict with INTEGER keys. Note that once written to JSON the keys
    become strings, so always reload via load_class_weights() (below) before
    handing them to Keras.
    """
...
def load_class_weights(path: str | Path = OUTPUT_DIR / "class_weights.json") -> Dict[int, float]:
    """
    Load class_weights.json back into a dict with INTEGER keys.

    JSON object keys are always strings, but Keras matches class_weight keys
    against the integer label ids emitted by the dataset. Passing the raw
    json.load() result (string keys) makes Keras silently train unweighted.
    Use this helper wherever the weights are consumed:

        cw = load_class_weights("preprocessing_output/class_weights.json")
        model.fit(train_ds, class_weight=cw, ...)
    """
...
def preprocess_image_opencv(
    ...
    image_size is (height, width). cv2.resize expects (width, height), so the
    axes are swapped explicitly below.
...
def _resolve_normalization(
    ...
    """
    Resolve the normalisation mode, keeping backward compatibility with the
    old boolean flag. Explicit `normalization` wins; otherwise the legacy
    `use_imagenet_normalization=True` maps to the torch-style "imagenet" mode.
    """
    ...
    return "imagenet" if use_imagenet_normalization else DEFAULT_NORMALIZATION
...
def build_tf_dataset(
    ...
    use_imagenet_normalization: bool = False,  # deprecated alias, see below
):
    """
    ...
    normalization (preferred):
        "zero_one" -> [0, 1]     (from-scratch CNN / dense network)
        "tf"       -> [-1, 1]    (keras.applications backbones, e.g. MobileNetV2)
        "imagenet" -> torch-style (x - mean) / std on [0, 1] inputs

    use_imagenet_normalization (deprecated):
        Kept for backward compatibility. If True and `normalization` is not
        given, it maps to "imagenet". Prefer passing `normalization` directly.
        For a Keras MobileNetV2 transfer model use normalization="tf".
    """
    import tensorflow as tf
    ...
    norm_mode = _resolve_normalization(normalization, use_imagenet_normalization)

    dataset_root = tf.constant(str(dataset_path))
    image_height, image_width = image_size

    # Materialise as plain numpy so from_tensor_slices is happy under pandas>=3.0
    # (where Series.values may return a StringArray rather than an ndarray).
    paths = manifest_df["relative_path"].astype(str).to_numpy(dtype=object)
    labels = manifest_df["label_idx"].astype("int32").to_numpy()
    ...
    def load_and_preprocess(relative_path, label):
        # relative_path uses POSIX separators; join with "/" for tf.io.read_file.
        full_path = tf.strings.join([dataset_root, relative_path], separator="/")
        image_bytes = tf.io.read_file(full_path)
        # expand_animations=False keeps a static rank so tf.image.resize is happy.
        image = tf.io.decode_image(image_bytes, channels=3, expand_animations=False)
        image = tf.image.resize(image, [image_height, image_width])  # (H, W)

        # Work in [0, 1] for augmentation, then apply the final normalisation.
...
def save_preprocessing_artifacts(
    ...
    # JSON keys are strings; reload with load_class_weights() to get int keys
    # before passing to Keras model.fit(class_weight=...).
    ...
    config = {
        ...
        "normalization": {
            ...
            "modes": {
                "zero_one": "pixels scaled to [0, 1]",
                "tf": "pixels scaled to [-1, 1] (keras.applications backbones)",
                "imagenet": "torch-style (x - mean) / std on [0, 1] inputs",
            },
            "imagenet_torch_stats": {
                "mean": IMAGENET_MEAN,
                "std": IMAGENET_STD,
            },
        },
        "class_weights_note": (
            "keys are strings in JSON; reload via load_class_weights() for "
            "integer keys before model.fit(class_weight=...)"
        ),
...
# ---------------------------------------------------------------------
# CHANGELOG (relative to the original preprocessing.py)
# ---------------------------------------------------------------------
# 1. class_weights: added load_class_weights() so consumers get INT keys.
#    Keras model.fit(class_weight=...) silently ignores string keys, which
#    disabled balanced weighting for the rare classes (basophil/monocyte/ig).
# 2. build_tf_dataset: replaced the ambiguous use_imagenet_normalization flag
#    with an explicit `normalization` mode ("zero_one" | "tf" | "imagenet").
#    Use "tf" ([-1, 1]) for keras.applications backbones such as MobileNetV2;
#    the torch-style stats are still available but are not what those backbones
#    expect. The old flag still works for backward compatibility.
# 3. IMAGE_SIZE is documented and used consistently as (height, width);
#    preprocess_image_opencv now passes (width, height) to cv2.resize.
# 4. from_tensor_slices inputs are materialised with .to_numpy(...) so the
#    pipeline is robust under pandas >= 3.0 (StringArray). relative_path is
#    stored/rejoined with POSIX "/" separators for cross-platform safety.
